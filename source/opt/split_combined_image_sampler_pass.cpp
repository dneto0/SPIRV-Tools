...
spv_result_t SplitCombinedImageSamplerPass::RemapFunctions() {
    ...
    if (inst.opcode() == spv::Op::OpTypeFunction) {
      ...
      for (uint32_t i = 1; i < inst.NumInOperands(); i++) {
        ...
        if (combined_types_.find(param_type_id) != combined_types_.end()) {
          ...
          assert(sampler_type);
          inst.SetOperand(
              i, {{SPV_OPERAND_TYPE_TYPE_ID, sampler_type->result_id()}});
          inst.InsertOperand(
              i, {{SPV_OPERAND_TYPE_TYPE_ID, image_type->result_id()}});
  ...
  for (Function* fn : context()->module()) {
    ...
    fn->ForEachParam([&](Instruction* param) {
      ...
      if (combined_types_.find(param_type_id) != combined_types_.end()) {
        to_replace.append({param, nullptr, nullptr});
      }
    }
    if (to_replace.empty()) {
      continue;
    ...
    fn->RewriteParams([&](std::unique_ptr<Instruction>&& from_param, std::back_insert_iterator<Function::ParamList>& appender) {
      std::unique_ptr<Instruction> param{from_param};
      if (param->get() == next_to_replace->param) {
        ...
        auto sampler_param = MakeUnique<Instruction>(
            context(), spv::Op::OpFunctionParameter, sampler_type->result_id(),
            context->TakeNextId(), {});
